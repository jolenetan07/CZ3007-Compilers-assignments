...
public class StmtCodeGenerator extends Visitor<Void> {
	...
	public Void visitBreakStmt(BreakStmt nd) {
		...
		Unit stmt = j.newGotoStmt(breakTargets.get(whileStmt));
		units.add(stmt);
		return null;
	}
	...
	public Void visitWhileStmt(WhileStmt nd) {
		 ...
		 */
		/*
		 * generate nop instructions label0 and label1
		 * emit statement label0
		 * generate code for condition c
		 * emit statement c == 0 goto label1
		...
		*/
		...
		units.add(label0);
		Value c = ExprCodeGenerator.generate(nd.getExpr(), fcg);
		units.add(j.newIfStmt(j.newEqExpr(c, IntConstant.v(0)), label1));
